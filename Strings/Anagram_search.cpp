//permutation of a string is present in continous position/contiguous location....
//linear time
#include <bits/stdc++.h> 
using namespace std; 
...
bool isPresent(string &txt,string &pat){
    ...
    for(int i=pat.length();i<txt.length();i++){
        ...
        CT[txt[i]]++; //sliding to get next element count
        CT[txt[i-pat.length()]]--;// removing prev windows first character
    }
    return false;
}
